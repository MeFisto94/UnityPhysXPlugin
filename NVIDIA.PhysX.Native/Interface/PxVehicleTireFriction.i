CSHARP_ARRAYS(physx::PxVehicleDrivableSurfaceType, PxVehicleDrivableSurfaceType)
SIMPLIFY_ENUM_UINT(PxVehicleDrivableSurfaceType,
    eSURFACE_TYPE_UNKNOWN=0xffffffffU
);


// TODO: actually, SurfaceType is just a struct with one mType field, a uint.
// ABI wise, that's just a uint, but that may lead to SWIG completely omitting it.
// We'd still want that type, just to be used and then converted to uint or something.
// The enum is pretty boring. User should fill up from 0...0xFFFFFFFF with custom values.
/*struct PxVehicleDrivableSurfaceType
{
	%typemap(csbase) eSURFACE_TYPE_UNKNOWN "uint"
	enum
	{
...
public:
	...
	*/
	%apply PxMaterial* INPUT[] { PxMaterial** drivableSurfaceMaterials }
	%apply PxVehicleDrivableSurfaceType INPUT[] { PxVehicleDrivableSurfaceType const * drivableSurfaceTypes }
	void setup
